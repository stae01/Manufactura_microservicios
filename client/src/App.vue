...
// vista actual: 'dashboard' | 'reports'
...
<template>
  ...
  <div v-else class="min-h-screen bg-background flex flex-col">
    ...
    <main class="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <!-- Vista Panel (defectos + alertas) -->
      <div v-if="currentView === 'dashboard'" class="grid grid-cols-1 lg:grid-cols-2 gap-8 h-full">
        <!-- Columna izquierda: reporte de defectos -->
        <section class="h-full">
          <DefectForm :inspector-id="userId" />
        </section>

        <!-- Columna derecha: alertas (solo admin) / acceso restringido -->
      ...
      <div v-else>
        ...
        >
          ...
          <h3 class="text-lg font-bold text-slate-700 mb-2">Acceso Restringido</h3>
          <p class="text-secondary max-w-xs mx-auto">
            La sección de reportes está reservada exclusivamente para administradores.
          </p>
        </div>
      </div>
    </main>
